# Validation script for deepseekv3_2-raw-decoupled-from-hf: proves data runs through correctly.
#
# Uses the same shared data as deepseekv3_2-triton/validate.py (from data/sample_data.py).
#
# Run from the deepseekv3_2/ root directory:
...
def load_model(cfg, device="cuda"):
    """Instantiate a tiny DeepSeek-V3 model for validation."""
    model = DeepSeekV3ForCausalLM(cfg).to(device).to(torch.bfloat16)
    return model
...
def main():
    ...
    print(f"Validating: deepseekv3_2-raw-decoupled-from-hf")
    print()
