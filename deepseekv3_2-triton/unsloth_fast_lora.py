...
# DeepSeek-V3's MLA uses LoRA-style compression for queries:
#   q_a_proj: [7168 -> 1536] (compression)
#   q_b_proj: [1536 -> 128*192=24576] (expansion)
# This is structurally similar to LoRA but with trained (not frozen) weights.
#
# For fine-tuning, LoRA adapters can be applied to:
#   - q_a_proj, q_b_proj (query path)
#   - kv_a_proj_with_mqa (KV compression)
#   - o_proj (output projection)
#   - gate_proj, up_proj, down_proj (SwiGLU MLP)
#   - Expert gate_up_proj, down_proj (MoE experts -- via grouped LoRA)
#
...
def apply_lora_mlp_swiglu(self, X, inplace = True):
    """Apply fused LoRA + SwiGLU MLP.

    For DeepSeek-V3 dense layers: gate_proj/up_proj [7168->18432], down_proj [18432->7168]
    For DeepSeek-V3 MoE experts: gate_proj/up_proj [7168->2048], down_proj [2048->7168]
    """
...
class LoRA_QKV(torch.autograd.Function):
    ...
    For DeepSeek-V3 MLA, Q/K/V projections are:
    - q_a_proj [7168 -> 1536], q_b_proj [1536 -> 24576]
    - kv_a_proj [7168 -> 576]
    - kv_b_proj [512 -> 32768]
    """
...
class LoRA_W(torch.autograd.Function):
    ...
    For DeepSeek-V3: o_proj [128*128=16384 -> 7168]
    """
