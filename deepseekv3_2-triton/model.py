# DeepSeek-V3 full model -- assembles all components into a runnable model.
#
# This file contains the architectural scaffolding that connects the individual
# components (MLA attention, RoPE+YaRN, SwiGLU, MoE, RMSNorm, MTP) into
# a complete DeepSeek-V3 causal language model.
#
# Components imported from other files in this package:
#   mla_attention.py  -> MLAttention (attention, NO DSA)
#   rope_partial.py   -> RotaryEmbedding (with YaRN support)
#   cache.py          -> KVCache
#   config.py         -> DEEPSEEK_V3_CONFIG
#
# Paper ref: DeepSeek-V3 (arXiv 2412.19437)
# Architecture: 671B params, ~37B active, 61 layers (3 dense + 58 MoE),
#               MLA attention, YaRN RoPE, 256 experts, MTP (1 layer)
#
# KEY DIFFERENCES FROM GLM5 (arXiv 2602.15763v2):
# - No DSA (Dynamic Sparse Attention) -- GLM5-specific
# - YaRN RoPE scaling: factor=40, extending from 4096 to 163840
# - MoE routing: n_group=8, topk_group=4, topk_method="noaux_tc"
# - MTP: 1 layer (GLM5 has 3 shared MTP layers)
# - Dimensions: hidden=7168, heads=128, qk_nope=128, v_head=128
...
class FeedForward(nn.Module):
    ...
    Used in two places:
      1. Dense MLP layers (layers 0-2): intermediate_size = 18432
...
class TopkRouter(nn.Module):
    ...
    DeepSeek-V3 uses sigmoid activation (not softmax) and auxiliary-loss-free
    load balancing with an additive e_score_correction_bias.

    Paper ref: arXiv 2412.19437, Section 2.2 -- "Auxiliary-Loss-Free Load Balancing"
    The bias is added during expert selection but NOT during weight computation,
    allowing the model to balance load without distorting the routing signal.
...
class MoeExperts(nn.Module):
    ...
    DeepSeek-V3 config:
      num_experts = 256, hidden_dim = 7168, intermediate_dim = 2048
    ...
    def __init__(self, cfg):
        ...
        self.hidden_dim = cfg["hidden_size"]                  # 7168
...
class MoE(nn.Module):
    ...
    - scoring_func="sigmoid": sigmoid activation (not softmax)

    GLM5 uses n_group=1, topk_group=1 (no group-based selection).

    Architecture (arXiv 2412.19437, Section 2.2):
      1. Router computes per-expert logits via linear projection
      2. Sigmoid activation (not softmax) on logits
      3. Add e_score_correction_bias for selection (not weight computation)
      4. Group-based selection: pick top-4 groups, then top-8 experts within
      5. Normalize weights and scale by routed_scaling_factor (2.5)
      6. Route tokens through selected experts (weighted sum)
      7. Add shared expert output (always-active SwiGLU MLP)
    ...
    def __init__(self, cfg):
        ...
        self.n_group = cfg["n_group"]                      # 8
        self.topk_group = cfg["topk_group"]                # 4
    ...
    def route_tokens_to_experts(self, router_logits):
        ...
        DeepSeek-V3 routing algorithm (arXiv 2412.19437):
        1. Sigmoid on router logits (not softmax -- key design choice)
        2. Add e_score_correction_bias for selection (load balancing)
        3. Group experts into 8 groups of 32, score each group by top-2 sum
        4. Select top-4 groups
        5. Within selected groups, pick top-8 experts
        6. Normalize weights and scale by 2.5
        ...
        """
        # Sigmoid activation (not softmax -- key DeepSeek-V3 design choice)
        ...
        # Scale by routed_scaling_factor (2.5 for DeepSeek-V3)
...
class MTPLayer(nn.Module):
    ...
    KEY DIFFERENCE FROM GLM5:
    - DeepSeek-V3: 1 MTP layer (predicts 2 tokens total)
    - GLM5: 3 shared MTP layers (same weights applied 3 times, predicts 4 tokens)
    - GLM5's MTP is parameter-shared and acts as draft model for speculative decoding
      with acceptance length of 2.76 tokens

    Architecture (per MTP layer) -- arXiv 2412.19437, Section 2.3:
    - Shared embedding projection (maps token embeddings + hidden states)
    - One transformer-like block (norm + linear)
    - Output head projecting to vocabulary
    """
    ...
    def __init__(self, cfg):
        ...
        self.norm = RMSNorm(hidden_size, eps=cfg["rms_norm_eps"])
    ...
    def forward(self, hidden_states, embed_tokens, input_ids=None, labels=None):
        ...
        # Get embeddings of shifted input (positions 1..S-1)
        shifted_embeds = embed_tokens(input_ids[:, 1:])  # [B, S-1, hidden_size]
        truncated_hidden = hidden_states[:, :-1, :]       # [B, S-1, hidden_size]

        # Concatenate and project
        combined = torch.cat([truncated_hidden, shifted_embeds], dim=-1)  # [B, S-1, 2*hidden_size]
        mtp_hidden = self.embed_proj(combined)  # [B, S-1, hidden_size]
...
class DecoderLayer(nn.Module):
    ...
    The MLP is either a dense FeedForward (layers 0-2) or a full MoE (layers 3-60),
    determined by cfg["mlp_layer_types"][layer_idx].

    Args:
        cfg:       model config dict
        layer_idx: index of this layer (0-60)
    """

    def __init__(self, cfg, layer_idx):
        super().__init__()
        # MLA attention (no DSA indexer -- DeepSeek-V3 does not use DSA)
        self.self_attn = MLAttention(cfg, layer_idx)

        # Dense MLP for first 3 layers, MoE for the remaining 58
        if cfg["mlp_layer_types"][layer_idx] == "sparse":
            self.mlp = MoE(cfg)
        else:
            self.mlp = FeedForward(cfg)

        # Pre-norm layers (eps=1e-6 for DeepSeek-V3, not 1e-5 like GLM5)
...
# ---------------------------------------------------------------------------
# DeepSeekV3Model -- base model (embedding + 61 layers + final norm)
# ---------------------------------------------------------------------------

class DeepSeekV3Model(nn.Module):
    """DeepSeek-V3 base model: token embedding -> 61 decoder layers -> final RMSNorm.
    ...
    def __init__(self, cfg):
        ...
        # Token embedding: vocab_size -> hidden_size (129280 -> 7168)
        self.embed_tokens = nn.Embedding(cfg["vocab_size"], cfg["hidden_size"], self.padding_idx)

        # 61 decoder layers: first 3 dense, remaining 58 MoE
        ...
        # Rotary embedding with YaRN (computes cos/sin for the 64-dim rope portion)
    ...
    def forward(self, input_ids=None, attention_mask=None, position_ids=None,
        ...
        # Compute rotary embeddings once for all layers (with YaRN scaling)
        hidden_states = inputs_embeds
        position_embeddings = self.rotary_emb(hidden_states, position_ids)

        # Run through all 61 decoder layers
        for layer in self.layers:
            hidden_states = layer(
...
# ---------------------------------------------------------------------------
# DeepSeekV3ForCausalLM -- causal LM wrapper (base model + lm_head + MTP + loss)
# ---------------------------------------------------------------------------

class DeepSeekV3ForCausalLM(nn.Module):
    """DeepSeek-V3 causal language model: base model + linear LM head + MTP.
    ...
    Includes Multi-Token Prediction (MTP) auxiliary loss during training.
    ...
    def __init__(self, cfg):
        ...
        # LM head: hidden_size -> vocab_size (7168 -> 129280)
    ...
    def forward(self, input_ids=None, attention_mask=None, position_ids=None,
        ...
        Args:
            ...
            (other args forwarded to DeepSeekV3Model)
