...
# DeepSeek-V3 uses RMSNorm with eps=1e-6 (GLM5 uses eps=1e-5).
# RMSNorm is applied at:
#   - input_layernorm (pre-attention): dim=7168
#   - post_attention_layernorm (pre-MLP): dim=7168
#   - q_a_layernorm (MLA query compression): dim=1536
#   - kv_a_layernorm (MLA KV compression): dim=512
#   - final model norm: dim=7168
#   - MTP layer norm: dim=7168
#
# STATUS: Triton kernel -- production ready.
# The kernel fuses the FP32 variance computation, rsqrt, and weight
# multiplication into a single pass, avoiding 3 separate kernel launches
# and intermediate FP32 tensors.
#
# Original copyright:
...
def _rms_layernorm_forward(
    ...
    For DeepSeek-V3 with hidden_size=7168:
    - BLOCK_SIZE=8192, num_warps=16
    - eps=1e-6 (important: differs from GLM5's 1e-5)
    """
...
def fast_rms_layernorm(layernorm, X: torch.Tensor, gemma: bool = False):
    ...
    For DeepSeek-V3, eps=1e-6 (different from GLM5's 1e-5).
    """
