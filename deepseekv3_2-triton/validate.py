# Validation script for deepseekv3_2-triton.
#
# Run from the deepseekv3_2/ root:
#   python3 deepseekv3_2-triton/validate.py
#
# Tests (8 total):
...
#   7. Long sequence         -- 256-token sequence, attention survives (no DSA in DeepSeek-V3)
...
# NOTE: Unlike GLM5 validation, there is no DSA indexer to test.
# DeepSeek-V3 uses standard causal attention instead.
...
def load_model(cfg):
    model_mod = import_module("deepseekv3_2-triton.model")
    return model_mod.DeepSeekV3ForCausalLM(cfg).to(DEVICE).to(torch.bfloat16)
...
def test_backward_pass(model):
    ...
    if zero_names:
        print(f"  Zero-grad params: {zero_names[:5]}")
    # DeepSeek-V3 has no DSA indexer @no_grad, so ratio should be very high
    assert ratio > 0.7, f"  FAIL: only {ratio:.0%}"
    print(PASS)
    print()
    model.zero_grad()
...
# ---------------------------------------------------------------------------
# Test 7: Long sequence -- 256 tokens, stresses attention (no DSA for DeepSeek-V3)
# ---------------------------------------------------------------------------
