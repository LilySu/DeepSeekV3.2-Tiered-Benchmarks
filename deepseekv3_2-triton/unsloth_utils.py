# Standalone utility functions for DeepSeek-V3 Triton kernels.
...
# This file is model-agnostic -- the same utilities work for any MLA/MoE model.
#
# Original copyright:
...
def get_device_type():
    ...
    raise NotImplementedError(
        "deepseekv3_2-triton requires a CUDA, ROCm, or XPU GPU."
    )
...
def calculate_settings(n: int) -> tuple[int, int]:
    """Calculate optimal BLOCK_SIZE and num_warps for a given dimension.

    For DeepSeek-V3:
    - hidden_size=7168 -> BLOCK_SIZE=8192, num_warps=16
    - vocab_size=129280 -> exceeds MAX_FUSED_SIZE, needs chunked kernel
    - q_lora_rank=1536 -> BLOCK_SIZE=2048, num_warps=8
    - kv_lora_rank=512 -> BLOCK_SIZE=512, num_warps=4
    """
...
def get_lora_parameters(proj):
    """Extract base weight, quant state, and LoRA A/B/scaling from a projection.

    Works with HF PEFT LoRA layers. For DeepSeek-V3 MLA, the relevant
    projections are:
    - q_a_proj, q_b_proj (query LoRA path)
