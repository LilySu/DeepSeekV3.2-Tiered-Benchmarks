...
"""
Auto-tuning cache system for MoE kernels (FlashMLA+DeepGEMM variant).

Ensures kernel tuning runs only once at training start, then reuses cached
configurations for the duration of the training run and across restarts.
...
DeepSeek-V3 MoE dimensions for autotuning:
  ...
  - FP8 E4M3 with 128x128 block quantization
"""
...
def _get_cache_key(
    ...
    For DeepSeek-V3: num_experts=256, hidden_dim=7168, intermediate_dim=2048,
    top_k=8, use_deepgemm=True on H100.
    """
    key_data = {
...
def _get_cache_file_path(cache_key: str) -> str:
    ...
    return os.path.join(cache_dir, f"flashmla_deepgemm_{cache_key}.json")
...
def load_cached_config(cache_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Verify cache is still valid (same device, same libraries)
        current_device_capability = torch.cuda.get_device_capability()
        if cached_data.get("device_capability") != list(current_device_capability):
            logger.info("Device capability changed, invalidating cache")
            os.remove(cache_file)
            return None

        # Check if DeepGEMM availability changed
        has_deepgemm = _check_deepgemm_available()
        if cached_data.get("has_deepgemm", False) != has_deepgemm:
            logger.info("DeepGEMM availability changed, invalidating cache")
            os.remove(cache_file)
            return None

        logger.info(f"Loaded cached MoE kernel config (FlashMLA+DeepGEMM): {cache_key}")
        return cached_data
...
def save_cached_config(
    ...
    try:
        ...
        logger.info(f"Saved MoE kernel config cache (FlashMLA+DeepGEMM): {cache_key}")
    except Exception as e:
        logger.warning(f"Failed to save cache file {cache_file}: {e}")


def _check_deepgemm_available() -> bool:
    """Check if DeepGEMM library is available."""
    try:
        import deep_gemm
        return True
    except ImportError:
...
def get_or_autotune_moe_kernels(
    ...
    This variant specifically handles the FlashMLA+DeepGEMM pipeline:
    - If DeepGEMM is available and use_deepgemm=True, uses FP8 grouped GEMM
    - Otherwise falls back to Triton grouped GEMM kernels
    - FlashMLA handles the attention portion (not autotuned here)
    ...
    Args:
        ...
        use_deepgemm: Whether to use DeepGEMM FP8 kernels (requires SM90)
    ...
    cache_key = _get_cache_key(
        ...
        device_capability, seq_len, use_deepgemm,
    )

    # 0. Check environment variable override
    if os.environ.get("UNSLOTH_MOE_DISABLE_AUTOTUNE", "0") == "1":
        logger.info(
            f"UNSLOTH_MOE_DISABLE_AUTOTUNE=1: Using Heuristic (Safe) MoE kernel configs "
            f"for SM{device_capability[0]}{device_capability[1]} (FlashMLA+DeepGEMM variant)"
        )
        return _get_heuristic_configs(use_deepgemm=use_deepgemm)
    ...
    # 3. Run autotuning
    logger.info(f"Running MoE kernel auto-tuning (FlashMLA+DeepGEMM variant): {cache_key}")
    logger.info(
        f"Configuration: {num_experts} experts, {hidden_dim} hidden, "
        f"{intermediate_dim} intermediate, top_k={top_k}, "
        f"deepgemm={'enabled' if use_deepgemm else 'disabled'}"
    )
    ...
    try:
        ...
        if "AttributeError" in str(e) and "_experimental_make_tensor_descriptor" in str(e):
            logger.warning(
                "Unsloth: Your Triton version might be incompatible with TMA features. "
                "Falling back to default configs."
            )
        logger.info("Falling back to default kernel configurations")
        return _get_default_configs(use_deepgemm=use_deepgemm)
...
def _run_moe_autotuning(
    ...
    For DeepSeek-V3 with FlashMLA+DeepGEMM:
    - Creates dummy tensors matching 256-expert, 7168-hidden, 2048-intermediate
    - If DeepGEMM available, tunes FP8 grouped GEMM parameters
    - Otherwise tunes Triton grouped GEMM kernels
    """
    device = "cuda"
    # Fixed safe token count for autotuning (avoids OOM)
    num_tokens = 4096
    ...
    # Create balanced routing for tuning
    ...
    logger.info("Autotuning forward kernel (FlashMLA+DeepGEMM variant)...")
    _ = grouped_gemm_forward(
        X=hidden_states, W=gate_up_weights, topk=top_k, m_sizes=m_sizes,
    ...
    _ = grouped_gemm_dX(
        dY=dummy_grad, W=gate_up_weights, gather_indices=gather_indices,
    ...
    _ = grouped_gemm_dW(
        X=hidden_states, dY=dummy_grad, m_sizes=m_sizes, gather_indices=gather_indices,
...
def _get_heuristic_configs(use_deepgemm: bool = True) -> Tuple[Any, Any, Any]:
    """
    Get 'Safe Heuristic' kernel configurations for DeepSeek-V3 MoE.

    These are tuned for H100 SM90 with the FlashMLA+DeepGEMM pipeline.
    When DeepGEMM is used for forward, the Triton kernel config applies
    ...
    # Forward: 64x128x128 (conservative, fits A100 and H100 SMEM)
    config_fwd = KernelConfigForward(
        BLOCK_SIZE_M=64,
        BLOCK_SIZE_N=128,
        BLOCK_SIZE_K=128,
...
def _get_default_configs(use_deepgemm: bool = True) -> Tuple[Any, Any, Any]:
    ...
    logger.warning("Using default MoE kernel configurations (FlashMLA+DeepGEMM variant, not optimal)")
...
def clear_cache() -> None:
    ...
    logger.info("Cleared MoE kernel cache (FlashMLA+DeepGEMM variant)")
