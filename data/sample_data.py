# Shared sample data for validating DeepSeek-V3 forward/backward passes.
#
# This module provides synthetic token sequences that follow DeepSeek-V3's
# chat format. The token IDs are fake (the model has random weights anyway)
# but the structure is correct: special token boundaries, user/assistant turns,
# and properly masked labels.
#
# Used by both:
#   - deepseekv3_2-triton/validate.py
#   - deepseekv3_2-raw-decoupled-from-hf/validate.py
#
...
# ---------------------------------------------------------------------------
# Fake special token IDs (matching DeepSeek-V3 chat structure)
# ---------------------------------------------------------------------------
BOS_ID = 0           # <|begin_of_sentence|>
EOS_ID = 1           # <|end_of_sentence|>
USER_ID = 10         # <|User|>
ASSISTANT_ID = 11    # <|Assistant|>
SYSTEM_ID = 12       # <|System|>
NEWLINE_ID = 13      # \n
...
def make_chat_turn(role_id, content_length):
    """Build one chat turn: <role_token> content
    ...
    tokens = [role_id, NEWLINE_ID] + content + [NEWLINE_ID]
...
def make_conversation(system_len=3, user_len=8, assistant_len=6):
    """Build a single-turn conversation (system + user + assistant).
    ...
    input_ids = [BOS_ID] + sys_tokens + usr_tokens + ast_tokens + [EOS_ID]
    is_assistant = [False] + sys_mask + usr_mask + ast_mask + [False]
...
def get_tiny_config():
    """Return a minimal DeepSeek-V3 config that fits on any GPU.
